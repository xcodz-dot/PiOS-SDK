...
def main(arguments=None):
    ...
    parser = argparse.ArgumentParser(
        description="A tool to generate sha256 checksums for ppa, py, text, etc files"
    )

    options = parser.add_mutually_exclusive_group()
    options.add_argument(
        "-u", "--unicode", dest="mode", action="store_const", const="u"
    )
    ...
    parser.add_argument(
        "file",
        metavar="FILE",
        help="File is required to check for sha256",
        nargs="+",
        default=[],
    )
